...
def test_record_many_supports_multiple_executions():
    ...
    store.record_many([first, second, third])

    assert store.get_events("exec-001") == [first, third]
...
def test_record_many_rejects_invalid_event():
    ...
    with pytest.raises(TypeError):
        store.record_many([make_event(), "invalid"])
...
def test_step_id_validation():
    ...
    with pytest.raises(TypeError):
        store.get_step_events("exec-001", 123)
...
def test_empty_step_id_validation():
    ...
    with pytest.raises(ValueError):
        store.get_step_events("exec-001", "")
