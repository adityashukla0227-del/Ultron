...
def handle_command(user):
    ...
    # Incomplete profile commands
    if user == "set my" or (
        user.startswith("set my ") and " to " not in user
    ):
        print("\nUltron: Usage -> set my KEY to VALUE\n")
        return True

    if user == "change my" or (
    ...
    # Conversation context for identity queries
    elif user in [
        "what is my name",
        "who am i"
    ]:
        handle_conversation(user)
        return True
